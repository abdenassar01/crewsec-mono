...
export function FelparkeringResolveSheet({ id, isResolved }: Props) {
  ...
  const handleResolve = async () => {
    try {
      ...
      showMessage({
        type: 'success',
        message: 'Felparkering resolved successfully',
      });
      dismiss();
      setNotes('');
    } catch (error) {
      showMessage({
        ...
        message:
          error instanceof Error
            ? error.message
            : 'Failed to resolve felparkering',
      });
    }
  };
      ...
      <Modal index={0} snapPoints={['35%', '50%']} detached ref={ref}>
        <View className="justify-center p-4">
          ...
          <View className="mb-4 flex-row items-center gap-3">
            <View className="rounded-full bg-orange-500/20 p-2">
              <Image
                className="size-5"
          ...
          <View className="mb-4 gap-3">
            ...
            <Input
              sheet
              className="min-h-20 rounded-xl border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-700 dark:bg-gray-800"
