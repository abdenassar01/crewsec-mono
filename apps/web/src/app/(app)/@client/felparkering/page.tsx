...
import { Search01FreeIcons, CheckmarkCircle02FreeIcons, AlertCircleFreeIcons, ArrowLeft01FreeIcons } from '@hugeicons/core-free-icons';
...
export default function FelparkeringPage() {
  ...
  const handleCheck = async () => {
    ...
    if (exists) {
      toast.info('Detta fordon är redan registrerat som felparkering');
      return;
    }
    ...
    if (result) {
      toast.success('Felparkering har rapporterats');
      setCheckingReference('');
    } else {
      toast.error('Kunde inte rapportera felparkering');
    }
  };
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center text-card-foreground gap-3 w-fit bg-primary/10 rounded-full p-2 pr-5  hover:bg-primary/50">
              ...
              <div>
                <h1 className="text-sm font-medium ">Felparkering</h1>
                <p className="text-xs text-muted-foreground">Rapportera felparkerade fordon</p>
              </div>
            </div>
            <ModeToggle />
          </div>
          ...
          <div className="bg-gradient-to-r from-red-50 to-red-100/50 rounded-2xl p-5 border border-red-200">
            ...
            <div className="flex gap-3">
              <div className="flex-1 relative">
                ...
                <Input
                  ...
                  className="pl-10 bg-card border-red-200 h-12 text-lg font-semibold tracking-wider"
                />
              </div>
              <Button
                onClick={handleCheck}
                disabled={!checkingReference}
                className="bg-red-600 hover:bg-red-700 px-6 h-12 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {isLoading ? (
          ...
          <div className="bg-card rounded-2xl border border-border p-8 text-center">
            <div className="w-16 h-16 bg-muted rounded-2xl flex items-center justify-center mx-auto mb-4">
              <HugeiconsIcon icon={AlertCircleFreeIcons} className="w-8 h-8 text-muted-foreground" />
            </div>
            <h3 className="text-lg font-semibold text-card-foreground mb-2">
              Inga rapporterade felparkeringar
            </h3>
            <p className="text-sm text-muted-foreground">
              Rapportera en felparkering genom att ange registreringsnumret ovan
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-sm font-semibold text-muted-foreground">
                {violationList.length} {violationList.length === 1 ? 'rapport' : 'rapporter'}
              </h2>
            </div>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="w-12 h-12 bg-red-100 rounded-xl flex items-center justify-center flex-shrink-0">
                      <HugeiconsIcon icon={AlertCircleFreeIcons} className="w-6 h-6 text-red-600" />
                    </div>
